...
const ItemDefinitions: ItemCatalog = preload("res://game/content/items/catalog.tres")
	...
	for object_group in parsed.object_groups:
		for object in object_group.objects:
			...
			if absf(float(object.get("width", expected_width)) - expected_width) > 0.01 or absf(float(object.get("height", expected_height)) - expected_height) > 0.01:
				...
				return ERR_INVALID_DATA
			if object.properties.has("inventory_item_id"):
				var item_id := StringName(str(object.properties.inventory_item_id))
				if not ItemDefinitions.has_item(item_id):
					push_error("Objet ramassable '%s' lié à un item_id inconnu : %s" % [object.get("name", object_gid), item_id])
					return ERR_INVALID_DATA
				if int(object.properties.get("inventory_quantity", 1)) <= 0:
					push_error("Quantité de ramassage invalide pour '%s'." % object.get("name", object_gid))
					return ERR_INVALID_DATA
...
static func _convert_psychokinetic_tile_object(object: Dictionary, parent: Node, owner: Node, profile, tile_set: TileSet, resolved: Dictionary) -> void:
	...
	if object.properties.has("inventory_item_id"):
		var pickup := InventoryPickup.new()
		pickup.name = "Pickup"
		pickup.item = ItemDefinitions.get_item(StringName(str(object.properties.inventory_item_id)))
		pickup.quantity = int(object.properties.get("inventory_quantity", 1))
		pickup.instance_id = StringName(str(object.properties.get("inventory_instance_id", "")))
		_add_owned(body, pickup, owner)
