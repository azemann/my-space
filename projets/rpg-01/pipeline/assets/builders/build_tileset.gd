...
func _add_object_source(tile_set: TileSet, texture: Texture2D, metadata: Dictionary, source_id: int, source_name: String) -> void:
	...
	for sprite in metadata["sprites"]:
		...
		if str(sprite.get("anchor_kind", "foot")) == "foot":
			...
			# TileData.texture_origin décrit le point d'origine dans la texture ;
			# Godot le soustrait au rendu. Le signe doit donc aller de l'ancre
			# visuelle vers le centre, et non l'inverse.
			data.texture_origin = foot_anchor - Vector2i(region_size.x / 2, region_size.y / 2)
