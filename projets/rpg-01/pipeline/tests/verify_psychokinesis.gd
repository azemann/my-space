...
func _run() -> void:
	...
	# Le calque de premier plan ne doit pas réassigner son transform chaque frame :
	# cela autorise la lévitation visuelle mais bloque le mouvement du RigidBody2D.
	var moved_wreck := game.current_map.get_node("World/PlacedObjects/ForegroundObjects/PremierPlanEpaveProche") as PsychokineticBody2D
	var moved_wreck_origin := moved_wreck.global_position
	moved_wreck.effects_enabled = false
	moved_wreck.collision_mask = 0
	moved_wreck.begin_hold(moved_wreck_origin)
	moved_wreck.move_held(moved_wreck_origin + Vector2(64.0, 0.0), 0.0)
	for frame in 30:
		await physics_frame
	assert(moved_wreck.global_position.x > moved_wreck_origin.x + 48.0, "Un objet déplacé dans ForegroundObjects doit continuer de suivre la télékinésie")
	moved_wreck.restore_persistent_state(moved_wreck_origin, 0.0)
