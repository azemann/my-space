#!/usr/bin/env python3
"""Build level 3 with the original 32x32 ImageGen greenhouse atlas."""
...
MAP_PATH = ROOT / "maps" / "niveau-03-automates.tmx"
...
def build_layers():
    ...
    for y in range(10, 13):
        put(gameplay, 36, y, 58)
    put(gameplay, 27, 19, 52)
    put(gameplay, 25, 19, 48)
    put(gameplay, 26, 19, 49)
    put(gameplay, 34, 12, 60)
    put(gameplay, 23, 12, 60)
    put(gameplay, 10, 15, 60)
    put(gameplay, 43, 7, 60)
    put(gameplay, 47, 5, 55)
    put(gameplay, 46, 5, 53)
...
def main():
    ...
    properties(
        ...
        {
            "level_id": "serre-03",
            "level_name": "La nef des automates",
            "music": "serre_automates",
            "pixel_perfect": True,
            "template_version": 1,
        },
    )
    ET.SubElement(root, "tileset", firstgid="1", source="../assets/tilesets/niveau-03-serre-mecanique-32x32-v002.tsx")
    ...
    for object_id, (x1, x2, y, _family) in enumerate(
        [(2, 6, 18, "metal"), (7, 11, 16, "moss"), (12, 16, 14, "metal"),
         (16, 20, 11, "moss"), (21, 25, 13, "metal"), (26, 31, 15, "moss"),
         (32, 36, 13, "metal"), (36, 40, 10, "moss"), (41, 45, 8, "metal"),
         (44, 48, 6, "moss")],
        start=10,
    ):
    ...
    rectangle(movements, 31, "chaine_automate", "climbable", 36 * TILE, 10 * TILE, TILE, 3 * TILE)
    rectangle(movements, 32, "ressort_horloger", "bounce", 27 * TILE, 19 * TILE, TILE, TILE, {"impulse_y": -620.0})
    ...
    point(entities, 51, "checkpoint_automates", "checkpoint", 30 * TILE, 640, {"checkpoint_id": "cp_automates", "spawn_offset_y": -24.0})
    for object_id, x, y in ((60, 10, 15), (61, 23, 12), (62, 34, 12), (63, 43, 7)):
        point(entities, object_id, f"rouage_{object_id - 59:02d}", "collectible", x * TILE + 16, y * TILE, {"value": 1})

    interactions = object_group(root, 14, "14 — Interactions", "gameplay", "#c58cff")
    rectangle(interactions, 70, "levier_automate", "interactable", 46 * TILE, 5 * TILE, TILE, TILE, {"action": "open_exit", "target": "porte_automate"})
    rectangle(interactions, 71, "porte_automate", "exit", 47 * TILE, 5 * TILE, TILE, TILE, {"requires": "open_exit", "next_level": "niveau-04-a-definir"})
    ...
    rectangle(camera, 81, "focus_automates", "camera_focus", 34 * TILE, 5 * TILE, 15 * TILE, 9 * TILE, {"priority": 1, "zoom": 0.95})

    audio = object_group(root, 16, "16 — Audio", "audio", "#5cc8ff", visible=False)
    rectangle(audio, 90, "ambiance_nef", "audio_zone", 0, 0, 30 * TILE, 768, {"snapshot": "glasshouse_hum", "fade_time": 0.8})
    rectangle(audio, 91, "ambiance_automates", "audio_zone", 30 * TILE, 0, 20 * TILE, 768, {"snapshot": "clockwork_room", "fade_time": 0.8})
